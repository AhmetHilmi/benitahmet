...
app.get("/", (request, response) => {
  console.log(Date.now() + "Benita 7/24 Sistemi Aktif");
...
.setDescription(`<a:hyper01:751547082560634910> <@!${message.author.id}> adlı kullanıcı tarafından <#${message.channel.id}> kanalına gönderilen mesaj silindi!\n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Benita Bot | Log Sistemi")
...
if (!modlogkanal) return;
  ...
  .setColor("#ffd100")
    ...
    .setDescription(`<a:hyper01:751547082560634910> Üye Sunucudan Yasaklandı! \n<@!${message.user.id}>, ${message.user.tag}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                let embed = new Discord.RichEmbed()
                    .setColor('#ffd100')
                .setDescription(`<a:hyper01:751547082560634910> ${channel.name} adlı metin kanalı oluşturuldu.`)
                .setFooter(`Benita Bot | Log Sistemi Kanal ID: ${channel.id}`)
...
.setTitle("SES KANALI OLUŞTURULDU")
                .setDescription(`<a:hyper01:751547082560634910> ${channel.name} adlı ses kanalı oluşturuldu!`)
                .setFooter(`Benita Bot | Log Sistemi Kanal ID: ${channel.id}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                let embed = new Discord.RichEmbed()
                    .setColor('#ffd100')
                .setDescription(`<a:hyper01:751547082560634910> ${channel.name} adlı metin kanalı silini!`)
                .setFooter(`Benita Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
            if (channel.type === "voice") {
...
.setTitle("SES KANALI SİLİNDİ")
                .setDescription(`<a:hyper01:751547082560634910> ${channel.name} adlı ses kanalı silindi`)
            .setFooter(`Benita Bot | Log Sistemi  Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            }
    })
              ...
              if (!message.member.hasPermission("ADMINISTRATOR")) {
                  ...
                  if (uyarisayisi === null) {
                      let uyari = new Discord.RichEmbed()
                          .setColor("#ffd100")
                          .setFooter('Benita BOT', client.user.avatarURL)
                          .setDescription(`<a:hyper01:751547082560634910> **<@${message.author.id}> Reklam Kick Sistemine Yakalandın! Reklam Yapmaya Devam Edersen Kickleniceksin (1/3)**`)
                          .setTimestamp()
                  ...
                  if (uyarisayisi === 1) {
                      let uyari = new Discord.RichEmbed()
                          .setColor("#ffd100")
                          .setFooter('Benita BOT ', client.user.avatarURL)
                          .setDescription(`<a:hyper01:751547082560634910> **<@${message.author.id}> Reklam Kick Sistemine Yakalandın! Reklam Yapmaya Devam Edersen Kickleniceksin (2/3)**`)
                          .setTimestamp()
                      message.channel.send(uyari)
                  }
                  if (uyarisayisi === 2) {
                      ...
                      let uyari = new Discord.RichEmbed()
                          .setColor("#ffd100")
                          .setFooter('Benita BOT', client.user.avatarURL)
                          .setDescription(`<a:hyper01:751547082560634910> **<@${message.author.id}> 3 Adet Reklam Uyarısı Aldığı İçin Kicklendi. Bir Kez Daha Yaparsa Banlanacak**`)
                          .setTimestamp()
                      message.channel.send(uyari)
                  }
                  if (uyarisayisi === 3) {
                      message.delete();
                      await kullanici.ban({
                          reason: `Benita Reklam Kick Sistemi`,
                      })
                      db.delete(`reklamuyari_${message.author.id}`)
                      let uyari = new Discord.RichEmbed()
                          .setColor("#ffd100")
                          .setFooter('Benita BOT', client.user.avatarURL)
                          .setDescription(`<a:hyper01:751547082560634910> **<@${message.author.id}> Kick Yedikten Sonra Tekrar Devam Ettiği İçin Banlandı.**`)
                          .setTimestamp()
                      message.channel.send(uyari)
                  }
...
if (!role) {
  member.addRole(member.guild.roles.get(otorol))
                        ...
                        .setDescription("<a:hyper01:751547082560634910> **Sunucuya Yeni Katılan** @" + member.user.tag + " **Kullanıcısına** <@&" + otorol + ">  **Rolü verildi**")
                        .setColor('#ffd100') 
                        .setFooter(`Benita Otorol Sistemi`)
     member.guild.channels.get(i).send(embed) 
} else if (role) {
    member.addRole(member.guild.roles.get(otorol))
                        var embed = new Discord.RichEmbed()
                        .setDescription(`<a:hyper01:751547082560634910> **Sunucuya Yeni Katılan** \`${member.user.tag}\` **Kullanıcısına** \`${role}\` **Rolü verildi**`)
                        .setColor('#ffd100') 
                        .setFooter(`Benita Otorol Sistemi`)
     member.guild.channels.get(i).send(embed) 
